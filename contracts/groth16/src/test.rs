...
fn deploy(env: &Env, num_inputs: u32) -> Groth16ContractClient<'_> {
    let id = env.register_contract(None, Groth16Contract);
    let client = Groth16ContractClient::new(env, &id);
...
fn rejects_wrong_input_count() {
    ...
    expect_err(res, Groth16Error::PublicInputCountMismatch);
}
...
fn rejects_short_proof() {
    ...
    expect_err(res, Groth16Error::InvalidProofLength);
}
...
fn rejects_uninitialised_contract() {
    ...
    expect_err(res, Groth16Error::NotInitialized);
}
...
fn double_initialize_rejected() {
    ...
    assert_eq!(res, Err(Ok(Groth16Error::AlreadyInitialized)));
}
